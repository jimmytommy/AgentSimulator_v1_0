...
namespace Engine
{
    public class Environment
    {
        ...
        public static Environment loadEnvironment(string name)
        {
            ...
            Console.WriteLine("# prey: " + k.preys.Count);
        ...
        public Prey findPreyByName(string name)
        {
        ...
        //Returns the number of preys captured in this update
        public int updatePrey(List<Robot> robots, double timestep)
        {
            int captured = 0;
            foreach (Prey prey in preys)
            {
                if (!prey.caught)
                {
                    prey.update(robots, timestep);
                    if (prey.caught) captured++;
                }
            }
            return captured;
        }
