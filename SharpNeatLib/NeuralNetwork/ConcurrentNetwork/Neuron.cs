...
namespace SharpNeatLib.NeuralNetwork
{
	...
	public class Neuron 
	{
		...
		public void Recalc()
		{
			// No recalculation required for input, recieve, or bias nodes. 
			if(neuronType==NeuronType.Input || neuronType==NeuronType.Bias || neuronType==NeuronType.Receive)
				return;
...
//			outputRecalc = 1.0      *  Math.Tanh(0.2 * accumulator);
		...
		{
			// No recalculation required for input, receive or bias nodes. 
			if(neuronType==NeuronType.Input || neuronType==NeuronType.Bias || NeuronType==NeuronType.Receive)
				return;
